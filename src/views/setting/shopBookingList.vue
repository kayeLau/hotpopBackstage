    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="period" label="訂座時段">
        <template #default="scope">
          <p v-for="(item,index) of tableData[scope.$index].period" :key="index">{{item}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="mealTime" label="用餐時間">
        <template #default="scope">
          <p v-for="(item,index) of tableData[scope.$index].mealTime" :key="index">{{item}}</p>
        </template>
      </el-table-column>
      <el-table-column prop="person" label="位">
        <template #default="scope">
          <p v-for="(item,index) of tableData[scope.$index].person" :key="index">{{item}}</p>
        </template>
      </el-table-column>
    ...
    function getLocalData() {
      if (localData) {
        for (let [key, value] of Object.entries(localData)) {
          ...
          for (let i = 4; i < localData[key].length; i++) {
            ...
            if (value[i] && value[i].name === "訂座時段" && value[i].children) {
              ...
              for (let item of value[i].children) {
                let val =
                  item.value === 0 || item.value === [0, 0] ? "" : item.value;
          ...
          console.log(tableData[0].period);
        }
      }
    }
